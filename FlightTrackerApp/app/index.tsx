...
// sheet tint is the only thing that uses it, and that modal has not moved out
// yet. It goes with pm when it does.
...
function DragMirror({ from, to }: { from: SharedValue<number>; to: SharedValue<number> }) {
...
export default function Index() {
  ...
  const showResult = () => {
    // BACK TO ZERO FIRST. cardExitX lives on this component, not on the result,
    // so a card thrown off screen leaves it holding a full screen width after
    // the tree unmounts — and the next result would mount already gone.
    cardExitX.value = 0;
  ...
  const handleToggleSave = async () => {
    ...
    setSavedFlights(result.flights);
    if (!result.ok) {
      // A TOAST NOW, because the thing that used to show this is gone. It was
  ...
  return (
    <View style={s.root}>
      ...
      {/* AT THE AIRPORT, IN FULL. The archive sheet's structure line for line —
          ...
          GUARDED ON `flight`, because this Modal lives outside the block that
          renders the card and would otherwise read fields off null for one frame
          if a card were ever cleared while it was open. */}
      <Modal
        visible={airportSheetOpen && flight !== null}
        transparent
        animationType="none"
        ...
        <Pressable style={s.routeCalScrim} onPress={closeAirportSheet}>
          ...
          >
            ...
            <Pressable style={s.sheetBody}>
              ...
              {/* THE GUARD AGAIN, for the type checker rather than for the
                  render: `visible` above already keeps this Modal shut while
                  flight is null, but TypeScript cannot narrow a field through a
                  prop, and the alternative is a `!` on all five reads below — the
                  prop pass and the four airport fields. */}
              {flight !== null && (
