...
Second, the app on the tester's phone is seven commits behind the repository. The build installed through TestFlight predates the notification staleness rule, the iOS 26 floor, the Apple-glass toasts, the tab bar inset fix and the whole of the Gmail screen rework. The next build has not been made because the previous one was rejected by Apple at submission for a reason that has not been read yet.
...
**Build 4 was rejected by Apple at submission and nobody has read why.** The build finished, uploaded and was accepted by the submission service, and Apple then refused it with no reason surfaced through the tooling. The detail is on a page that needs the owner's login. Until that is read, no new build can reach the tester, which is what blocks all seven pending commits.
